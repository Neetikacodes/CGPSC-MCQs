...
function renderQuestion(){
  ...
  const qTextWrap = el('q-text');
  qTextWrap.innerHTML = '';
  if(q.q_hi){
    const hiLine = document.createElement('div');
    hiLine.className = 'q-text-hi';
    hiLine.textContent = q.q_hi;
    qTextWrap.appendChild(hiLine);
  }
  const enLine = document.createElement('div');
  enLine.className = q.q_hi ? 'q-text-en' : '';
  enLine.textContent = q.q;
  qTextWrap.appendChild(enLine);
  ...
  ['A','B','C','D'].forEach(letter => {
    ...
    if(q.options_hi && q.options_hi[letter]){
      const hiSpan = document.createElement('div');
      hiSpan.className = 'option-text-hi';
      hiSpan.textContent = q.options_hi[letter];
      textWrap.appendChild(hiSpan);
      const enSpan = document.createElement('div');
      enSpan.className = 'option-text-en';
      enSpan.textContent = q.options[letter];
      textWrap.appendChild(enSpan);
    } else {
      textWrap.textContent = q.options[letter];
    }
  ...
  if(submitted){
    ...
    let html = '';
    if(q.explanation_hi) html += `<div class="explain-hi"><b>व्याख्या:</b> ${q.explanation_hi}</div>`;
    html += `<div class="explain-en"><b>Explanation:</b> ${q.explanation}</div>`;
...
function buildReviewList(){
  ...
  questions.forEach((q,i) => {
    ...
    const qHiLine = q.q_hi ? `<div style="color:var(--ink); margin-bottom:2px;">${i+1}. ${q.q_hi}</div>` : '';
    item.innerHTML = `
      <div class="q-text">${qHiLine}<span>${i+1}. ${q.q}</span> <span class="review-tag ${status}">${status}</span></div>
      ...
      ${q.explanation_hi ? `<div style="font-size:0.85rem; color:var(--graphite); margin-bottom:4px;"><b>व्याख्या:</b> ${q.explanation_hi}</div>` : ''}
